...
def generate_incident_report_pdf(case: dict) -> BytesIO:
    ...
    if case.get("anomalies"):
        ...
        for i, anomaly in enumerate(case.get("anomalies", []), 1):
    ...
    if case.get("response_actions"):
        for i, action in enumerate(case.get("response_actions", []), 1):
    ...
    if case.get("report"):
        ...
        # Preserve line breaks by splitting into paragraphs for robustness
...
def generate_pipeline_trace_pdf(case: dict) -> BytesIO:
    ...
    if case.get("pipeline_log"):
        for i, log_line in enumerate(case.get("pipeline_log", []), 1):
            ...
            color = colors.HexColor('#FF3B5C')  # Red for errors
            ...
            # Use a wrapped paragraph for the log line; color the bullet only to avoid invalid font color values
